...
## If you are planning to use this script on a shared platform make sure to save this key in a separate secured file.
...
if st.checkbox("Retrieve Password"):
    website_name = st.selectbox("Select website name:",
                                options=[""] + [row[0] for row in csv.reader(open('credentials.csv', 'r'))])
