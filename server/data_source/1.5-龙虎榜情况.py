...
for index, row in df.iterrows():
    ...
    # # 根据指定字段，进行left类型的合并（没有的字段位NaN）；将字符中指定部分去掉
    detail_and_hyyyb_and_traderstatistic_df = pd.merge(detail_and_hyyyb_df, lhb_traderstatistic_df, on='营业部名称',how='left')
    detail_and_hyyyb_and_traderstatistic_df['营业部名称'] = detail_and_hyyyb_and_traderstatistic_df['营业部名称'].str.replace('股份有限公司', '').str.replace('有限责任公司', '').str.replace('有限公司', '')
    print(tabulate(detail_and_hyyyb_and_traderstatistic_df, headers='keys', tablefmt='psql'))
    # # 营业部排行
    lhb_yybph_df = ak.stock_lhb_yybph_em(symbol='近一月')
    print(tabulate(lhb_yybph_df, headers='keys', tablefmt='psql'))
